...
const { circleno, NSFW1, boobs1, boobs2, boobs3, boobs4, NoGodNo} = require('../../config/emoji.json');

module.exports = {
    name: "BoobsFuck",
    aliases: ["Boobsfuck", "bbf", "boobsfuck"],
            ...
            try {
                async function getImage() {
                    ...
                    if (link.includes('redgifs')
                        ...
                        let Nsfw_embed = {
                            title: `${NSFW1}| Boobs Fuck || ${Responses[Response]}`,
                            fields: [
                                {
                                    name: 'Tytuł:',
                                    value: title,
                        ...
                        message.channel.send(link).then(embedMessage => {
                            ...
                            .then(() => embedMessage.react(boobs3))
                            .then(() => embedMessage.react(boobs4))
                    ...
                    let Nsfw_embed = new MessageEmbed()
                        .setColor(embedNsfw)
                        .setURL(link)
                        .setTitle(`${NSFW1}| Boobs Fuck || ${Responses[Response]}`)
